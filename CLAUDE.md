...
You are the **developer of agent-comm**, not an end-user agent. When working in this repo:

- Use **project-local paths**: `skills/agent-comm/scripts/ac`, not `~/.hermes/skills/agent-comm/scripts/ac`
- Real hostnames and agent names go in `.claude/CLAUDE.md` only (gitignored) — never in committed files
- The **agent-facing docs** (`skills/agent-comm/SKILL.md` + `references/`) are what gets deployed to agents — changes to integration flow must update these first
- When writing docs, assume the reader is a fresh agent that has never seen this project — it must be usable from the skill alone

## Documentation Rules (committed files only)

- **Never use real hostnames** (`.lan` addresses) or real agent names in any committed file
- **Use these placeholders**: `agent-comm-host` for LAN hostname, `agent-1`/`agent-2` for agents, `my-agent` for "your agent"
- **Never use `localhost`** in examples or defaults — remote agents can't reach it. Use `agent-comm-host:3420`
- **Never use role-based names** like `sender-agent`/`receiver-agent` — every agent is both. Use `agent-1`/`agent-2`
- **Verify integration flows against source code**, not memory — read the actual implementation before writing docs about it
...
- **Config variables**: `AC` (path to CLI), `COMM_USER` (agent name), `COMM_HOST` (server, default `agent-comm-host:3420`).
