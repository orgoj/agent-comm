...
- **Direct messages not showing in Messages view**: Upstream code filters DMs (`channel_id=null && to_agent!=null`) in two places: `ws.ts getMessagesData()` and `app.js handleEvent('message:sent')`. Our fork removes both filters. If you merge upstream changes, watch for these re-appearing.
